    ...
    <main>
        <div class="wrapper">
            ...
            <div class="add-wrapper full">
                <form action="{{route('diet.update', ['id' => $diet->id])}}" method="post" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                        ...
                        <div class="add-wrapper-item photo">
                            ...
                            <div class="add-wrapper-item-btn upload-photo">
                                <img id="picked-result" src="{{ $diet->imageUrl }}">
                        ...
                        <div class="add-wrapper-inner">
                            <div class="add-wrapper-item">
                                ...
                                <input name="title" type="text" class="@error('title') error @enderror" value="{{$diet->title}}" />
                                @error('title')
                                    <div class="error-message">{{$message}}</div>
                                @enderror
                            </div>
                    ...
                    @foreach($diet->data['days'] as $day)
                        ...
                        <button class="remove remove-ingredient">Remove Day</button>
                            ...
                            <div class="add-wrapper-item">
                                <label>Snack</label>
                                <textarea name="days-morning-snack[]" placeholder="morning snack">{{$day['morning-snack']}}</textarea>
                            </div>

                            <div class="add-wrapper-item">
                                <label>Breakfast</label>
                                <textarea name="days-breakfast[]" placeholder="e.g. Greek yogurt with strawberries and chia seeds">{{$day['breakfast']}}</textarea>
                            </div>

                            <div class="add-wrapper-item">
                                <label>Afternoon snack</label>
                                <textarea name="days-afternoon-snack[]" placeholder="afternoon snack">{{$day['afternoon-snack']}}</textarea>
                            </div>

                            <div class="add-wrapper-item">
                                <label>Lunch</label>
                                <textarea name="days-lunch[]" placeholder="e.g. A whole grain sandwich with hummus and vegetables">{{$day['lunch']}}</textarea>
                            </div>

                            <div class="add-wrapper-item">
                                <label>Dinner</label>
                                <textarea name="days-dinner[]" placeholder="e.g. A tuna salad with greens and olive oil, as well as a fruit salad">{{$day['dinner']}}</textarea>
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="add-wrapper-item add-diet-textarea">
                        ...
                        <textarea name="description" class="@error('description') error @enderror">{{$diet->description}}</textarea>
                        @error('description')
                            <div class="error-message">{{$message}}</div>
                        @enderror
                    </div>
                    ...
                    <div class="add-wrapper-item pdf">
                        ...
                        @if($diet->attachmentUrl)
                        <a href="{{$diet->attachmentUrl}}" target="_blank">Download attachment</a>
                        @endif
...
<script>
    $(document).ready(function () {
        ...
        $('.add-multiselect').val(@json($diet->codes)); 
